...
class TreeFrame(wx.Frame):
    ...
    def __init__(self, parent, command_q: Queue):
        ...
        # self.tree.Bind(wx.dataview.EVT_DATAVIEW_ITEM_BEGIN_DRAG, self.on_begin_drag)
        # self.tree.Bind(wx.dataview.EVT_DATAVIEW_ITEM_DROP, self.on_drop)
    ...
    def change_size(self, evt: wx.MouseEvent):
        ...
        self.tree.SetFont(self.font)
        new_list = wx.ImageList(self.image_size, self.image_size)

        self.tree.AssignImageList(new_list)

        for image in self.image_item:
            item = self.image_item[image]
            resized_image = image.Scale(self.image_size, self.image_size)

            del self.image_item[image]
            self.image_item[resized_image] = item
            image = new_list.Add(resized_image.ConvertToBitmap())
    ...
    def add_pic(self, item, name: str, is_folder: bool):
        ...
        item_image = wx.Image(self.cwd + pic_name, wx.BITMAP_TYPE_PNG).Scale(self.image_size, self.image_size)
        self.image_item[item_image] = item
    ...
    def copy_paste(self, evt: wx.KeyEvent):
        ...
        if ctrl_down:
            ...
            print('data:', self.tree.GetItemData(on_item))
            # copy
            if key_code == 67:
                print('copy')
    ...
    def on_drag(self, evt):
        ...
        """
        evt.Allow()
        cursor = wx.Cursor(wx.Image(r'T:\public\cyber\ophir\Reflection\Reflection\graphics\icons\txt.png'))
    ...
    def on_expanded(self, evt):
        ...
        """
        item = evt.GetItem()
        item_image = wx.Image(self.cwd + self.open_folder_name, wx.BITMAP_TYPE_PNG).Scale(self.image_size, self.image_size)
        self.image_item[item_image] = item
        item_image = item_image.ConvertToBitmap()
        item_image = self.image_list.Add(item_image)
        self.tree.SetItemImage(item, item_image, wx.TreeItemIcon_Normal)
    ...
    def on_collapsed(self, evt):
       ...
       """
        item = evt.GetItem()
        item_image = wx.Image(self.cwd + self.close_folder_name, wx.BITMAP_TYPE_PNG).Scale(self.image_size, self.image_size)
