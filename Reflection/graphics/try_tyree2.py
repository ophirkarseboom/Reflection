...
class MyFrame(wx.Frame):
    ...
    def extendTree(self, parentID):
        ...
        excludeDirs = ["d:", "/System Volume Information"]

        # retrieve the associated absolute path of the parent
        parentDir = self.tree.GetItemData(parentID)[0]

        subdirs = self.cached_listdir(parentDir)
        subdirs.sort()
        for child in subdirs:
            ...
            if os.path.isdir(child_path) and not os.path.islink(child):
                ...
                self.tree.SetItemData(childID, (child_path, False))
                ...
                newsubdirs = self.cached_listdir(newParentPath)
                newsubdirs.sort()
                for grandchild in newsubdirs:
                    ...
                    if os.path.isdir(grandchild_path) and not os.path.islink(grandchild_path):
                        ...
                        self.tree.SetItemData(grandchildID, (grandchild_path, False))

    def cached_listdir(self, path):
        res = self.global_cache.get(path)
        if res is None:
            res = os.listdir(path)
        self.global_cache[path] = res
        return res
